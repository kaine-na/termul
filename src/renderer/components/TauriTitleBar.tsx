					...
					<Tooltip>
						<TooltipTrigger asChild>
							<div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 cursor-pointer hover:bg-emerald-500/25 transition-colors">
								...
								<button
									...
									title={showDetails ? "Hide port" : "Show port"}
									aria-label={
										showDetails ? "Hide port details" : "Show port details"
									}
								>
									{showDetails ? (
										<EyeOff size={11} className="text-emerald-400/70" />
									) : (
										<Eye size={11} className="text-emerald-400/70" />
									)}
								</button>
							</div>
						</TooltipTrigger>
						<TooltipContent side="bottom" className="max-w-xs">
							<div className="space-y-2">
								...
								<p className="text-xs opacity-80 break-all font-mono leading-relaxed">
									{showDetails
										? remoteUrl
										: remoteUrl?.replace(/:\d+/, ":••••")}
								</p>
